...
export const alex_tools = TOOLS
  .filter(tool => tool.category === "alex")
  .map(tool => tool.id);

export const bitflow_tools = TOOLS
  .filter(tool => tool.category === "bitflow")
  .map(tool => tool.id);

export const lunarcrush_tools = TOOLS
  .filter(tool => tool.category === "lunarcrush")
  .map(tool => tool.id);

export const web_search_tools = TOOLS
  .filter(tool => tool.category === "web_search")
  .map(tool => tool.id);
